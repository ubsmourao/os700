...
def show_inventory_list():
    ...
    # 1) Filtro por texto (marca, modelo, número de série etc.)
...
Abaixo está um **exemplo** de como **remover** a função de **edição em massa** e **adicionar filtros** no `show_inventory_list()` para refinar a busca de máquinas cadastradas. Assim, você não terá mais a função `show_inventory_list_with_batch_edit`, e a listagem principal terá filtros de texto, status, UBS e setor.
...
###########################
# Dashboard do Inventário (exemplo opcional)
###########################
